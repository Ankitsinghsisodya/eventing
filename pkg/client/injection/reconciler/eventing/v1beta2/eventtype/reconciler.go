...
import (
	...
	eventingv1beta2 "knative.dev/eventing/pkg/client/listers/eventing/v1beta2"
...
// controller reconciling v1beta2.EventType.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1beta2.EventType. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1beta2.EventType) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1beta2.EventType.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1beta2.EventType. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1beta2.EventType) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1beta2.EventType if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1beta2.EventType.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1beta2.EventType) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1beta2.EventType) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1beta2.EventType resources.
type reconcilerImpl struct {
	...
	Lister eventingv1beta2.EventTypeLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister eventingv1beta2.EventTypeLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1beta2.EventType, desired *v1beta2.EventType) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.EventingV1beta2().EventTypes(desired.Namespace)
		...
		updater := r.Client.EventingV1beta2().EventTypes(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1beta2.EventType, desiredFinalizers sets.Set[string]) (*v1beta2.EventType, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1beta2.EventType, desiredFinalizers sets.Set[string]) (*v1beta2.EventType, error) {
	...
	patcher := r.Client.EventingV1beta2().EventTypes(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1beta2.EventType, desiredFinalizers sets.Set[string]) (*v1beta2.EventType, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1beta2.EventType) (*v1beta2.EventType, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1beta2.EventType, reconcileEvent reconciler.Event) (*v1beta2.EventType, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.EventingV1beta2().EventTypes(resource.Namespace)
