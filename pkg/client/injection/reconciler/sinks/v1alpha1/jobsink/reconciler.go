...
// controller reconciling v1alpha1.JobSink.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.JobSink. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1alpha1.JobSink) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.JobSink.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.JobSink. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.JobSink) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.JobSink if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.JobSink.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.JobSink) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.JobSink) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.JobSink resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sinksv1alpha1.JobSinkLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.JobSinks(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.JobSink, desired *v1alpha1.JobSink) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.SinksV1alpha1().JobSinks(desired.Namespace)
		...
		updater := r.Client.SinksV1alpha1().JobSinks(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.JobSink, desiredFinalizers sets.Set[string]) (*v1alpha1.JobSink, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1alpha1.JobSink, desiredFinalizers sets.Set[string]) (*v1alpha1.JobSink, error) {
	...
	patcher := r.Client.SinksV1alpha1().JobSinks(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1alpha1.JobSink, desiredFinalizers sets.Set[string]) (*v1alpha1.JobSink, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.JobSink) (*v1alpha1.JobSink, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.JobSink, reconcileEvent reconciler.Event) (*v1alpha1.JobSink, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.SinksV1alpha1().JobSinks(resource.Namespace)
