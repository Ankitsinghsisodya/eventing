...
// controller reconciling v1.Subscription.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.Subscription. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1.Subscription) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Subscription.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.Subscription. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Subscription) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Subscription if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Subscription.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.Subscription) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.Subscription) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.Subscription resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister messagingv1.SubscriptionLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.Subscription, desired *v1.Subscription) error {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Subscription, desiredFinalizers sets.Set[string]) (*v1.Subscription, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.Subscription, desiredFinalizers sets.Set[string]) (*v1.Subscription, error) {
	...
	patcher := r.Client.MessagingV1().Subscriptions(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.Subscription, desiredFinalizers sets.Set[string]) (*v1.Subscription, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Subscription) (*v1.Subscription, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Subscription, reconcileEvent reconciler.Event) (*v1.Subscription, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.MessagingV1().Subscriptions(resource.Namespace)
