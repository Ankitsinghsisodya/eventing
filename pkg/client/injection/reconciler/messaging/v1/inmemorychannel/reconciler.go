...
// controller reconciling v1.InMemoryChannel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.InMemoryChannel. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1.InMemoryChannel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.InMemoryChannel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.InMemoryChannel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.InMemoryChannel) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.InMemoryChannel if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.InMemoryChannel.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.InMemoryChannel) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.InMemoryChannel) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.InMemoryChannel resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister messagingv1.InMemoryChannelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.InMemoryChannel, desired *v1.InMemoryChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.MessagingV1().InMemoryChannels(desired.Namespace)
		...
		updater := r.Client.MessagingV1().InMemoryChannels(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.InMemoryChannel, desiredFinalizers sets.Set[string]) (*v1.InMemoryChannel, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.InMemoryChannel, desiredFinalizers sets.Set[string]) (*v1.InMemoryChannel, error) {
	...
	patcher := r.Client.MessagingV1().InMemoryChannels(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.InMemoryChannel, desiredFinalizers sets.Set[string]) (*v1.InMemoryChannel, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.InMemoryChannel) (*v1.InMemoryChannel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.InMemoryChannel, reconcileEvent reconciler.Event) (*v1.InMemoryChannel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.MessagingV1().InMemoryChannels(resource.Namespace)
