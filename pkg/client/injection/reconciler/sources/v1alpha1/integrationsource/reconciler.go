...
// controller reconciling v1alpha1.IntegrationSource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.IntegrationSource. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1alpha1.IntegrationSource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.IntegrationSource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.IntegrationSource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.IntegrationSource) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.IntegrationSource if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.IntegrationSource.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.IntegrationSource) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.IntegrationSource) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.IntegrationSource resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sourcesv1alpha1.IntegrationSourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.IntegrationSources(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.IntegrationSource, desired *v1alpha1.IntegrationSource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.SourcesV1alpha1().IntegrationSources(desired.Namespace)
		...
		updater := r.Client.SourcesV1alpha1().IntegrationSources(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.IntegrationSource, desiredFinalizers sets.Set[string]) (*v1alpha1.IntegrationSource, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1alpha1.IntegrationSource, desiredFinalizers sets.Set[string]) (*v1alpha1.IntegrationSource, error) {
	...
	patcher := r.Client.SourcesV1alpha1().IntegrationSources(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1alpha1.IntegrationSource, desiredFinalizers sets.Set[string]) (*v1alpha1.IntegrationSource, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.IntegrationSource) (*v1alpha1.IntegrationSource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.IntegrationSource, reconcileEvent reconciler.Event) (*v1alpha1.IntegrationSource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.SourcesV1alpha1().IntegrationSources(resource.Namespace)
