...
// controller reconciling v1.ApiServerSource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.ApiServerSource. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1.ApiServerSource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.ApiServerSource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.ApiServerSource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.ApiServerSource) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.ApiServerSource if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.ApiServerSource.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.ApiServerSource) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.ApiServerSource) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.ApiServerSource resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sourcesv1.ApiServerSourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.ApiServerSource, desired *v1.ApiServerSource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.SourcesV1().ApiServerSources(desired.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.ApiServerSource, desiredFinalizers sets.Set[string]) (*v1.ApiServerSource, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.ApiServerSource, desiredFinalizers sets.Set[string]) (*v1.ApiServerSource, error) {
	...
	patcher := r.Client.SourcesV1().ApiServerSources(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.ApiServerSource, desiredFinalizers sets.Set[string]) (*v1.ApiServerSource, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.ApiServerSource) (*v1.ApiServerSource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.ApiServerSource, reconcileEvent reconciler.Event) (*v1.ApiServerSource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.SourcesV1().ApiServerSources(resource.Namespace)
