...
// controller reconciling v1.PingSource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.PingSource. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1.PingSource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.PingSource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.PingSource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.PingSource) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.PingSource if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.PingSource.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.PingSource) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.PingSource) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.PingSource resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sourcesv1.PingSourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.PingSource, desired *v1.PingSource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.SourcesV1().PingSources(desired.Namespace)
		...
		updater := r.Client.SourcesV1().PingSources(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.PingSource, desiredFinalizers sets.Set[string]) (*v1.PingSource, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.PingSource, desiredFinalizers sets.Set[string]) (*v1.PingSource, error) {
	...
	patcher := r.Client.SourcesV1().PingSources(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.PingSource, desiredFinalizers sets.Set[string]) (*v1.PingSource, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.PingSource) (*v1.PingSource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.PingSource, reconcileEvent reconciler.Event) (*v1.PingSource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.SourcesV1().PingSources(resource.Namespace)
