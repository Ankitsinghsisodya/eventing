...
// controller reconciling v1.Sequence.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.Sequence. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1.Sequence) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Sequence.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.Sequence. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Sequence) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Sequence if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Sequence.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.Sequence) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.Sequence) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.Sequence resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister flowsv1.SequenceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.Sequence, desired *v1.Sequence) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.FlowsV1().Sequences(desired.Namespace)
		...
		updater := r.Client.FlowsV1().Sequences(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Sequence, desiredFinalizers sets.Set[string]) (*v1.Sequence, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.Sequence, desiredFinalizers sets.Set[string]) (*v1.Sequence, error) {
	...
	patcher := r.Client.FlowsV1().Sequences(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.Sequence, desiredFinalizers sets.Set[string]) (*v1.Sequence, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Sequence) (*v1.Sequence, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Sequence, reconcileEvent reconciler.Event) (*v1.Sequence, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.FlowsV1().Sequences(resource.Namespace)
