...
// controller reconciling v1.Parallel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.Parallel. Any changes
	...
	ReconcileKind(ctx context.Context, o *v1.Parallel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Parallel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.Parallel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Parallel) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Parallel if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Parallel.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.Parallel) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.Parallel) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.Parallel resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister flowsv1.ParallelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.Parallel, desired *v1.Parallel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.FlowsV1().Parallels(desired.Namespace)
		...
		updater := r.Client.FlowsV1().Parallels(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Parallel, desiredFinalizers sets.Set[string]) (*v1.Parallel, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.Parallel, desiredFinalizers sets.Set[string]) (*v1.Parallel, error) {
	...
	patcher := r.Client.FlowsV1().Parallels(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.Parallel, desiredFinalizers sets.Set[string]) (*v1.Parallel, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Parallel) (*v1.Parallel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Parallel, reconcileEvent reconciler.Event) (*v1.Parallel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.FlowsV1().Parallels(resource.Namespace)
